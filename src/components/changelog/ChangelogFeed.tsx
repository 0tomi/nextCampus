...
import { TutorialCarouselModal } from './TutorialCarouselModal'
...
export function ChangelogFeed({
  ...
  const [tutorialEntry, setTutorialEntry] = useState<ChangelogEntryView | null>(null)
  ...
  return (
    <>
      <div className={compact ? 'space-y-2' : 'stagger-children space-y-4'}>
        {entries.map((entry) => (
          <DarkCard
            ...
            onPointerEnter={() => {
              if (entry.unread) onMarkRead?.([entry.id])
            }}
          >
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              <button
                type="button"
                onClick={() => setTutorialEntry(entry)}
                className="inline-flex cursor-pointer items-center gap-2 rounded border border-white/10 bg-transparent px-3 py-1.5 text-xs font-semibold text-white/62 transition-colors hover:bg-white/5 hover:text-white"
              >
                Ver tutorial
              </button>
            </div>
      ...
      <TutorialCarouselModal
        changelogId={tutorialEntry?.changelogId ?? null}
        title={tutorialEntry ? `Tutorial: ${tutorialEntry.title}` : 'Tutorial'}
        open={Boolean(tutorialEntry)}
        onClose={() => setTutorialEntry(null)}
      />
    </>
  )
}
