...
export function ConfigurarForm({ careerName, years }: ConfigurarFormProps) {
  ...
  useEffect(() => {
    if (isHydrated) {
      setDraft(prefs ?? EMPTY_PREFERENCES)
    }
  }, [isHydrated, prefs])
              ...
              >
                ...
                >
                  <input
                    ...
                    disabled={!isHydrated}
                  />
                  {/* CheckBox span must be a sibling of the peer input to receive peer-focus-visible */}
                      ...
                      >
                        <label className="flex cursor-pointer items-center gap-3 px-5 py-3.5 transition-colors hover:bg-white/5">
                          <input
                            ...
                            disabled={!isHydrated || !yearChecked}
                          />
        ...
        <div className="mx-auto flex max-w-4xl items-center justify-between gap-3 px-4 py-4 sm:px-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              onClick={onResetAll}
              disabled={!isHydrated}
              className="cursor-pointer rounded-md border border-white/10 px-4 py-2.5 text-sm text-white/70 transition-colors hover:bg-white/5 hover:text-white disabled:cursor-not-allowed disabled:opacity-40"
            >
              Mostrar todo
            </button>
            <button
              ...
              onClick={onSave}
              disabled={!isHydrated}
              className="cursor-pointer rounded-md bg-uader-red px-5 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-uader-red-light disabled:cursor-not-allowed disabled:opacity-40"
            >
              Guardar
            </button>
          </div>
