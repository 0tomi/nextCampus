  ...
  if (phase === 'done') {
    ...
    return (
      <div className="space-y-4">
        ...
        <UnitBreakdown resultados={Object.values(resultados)} />
...
// Agrupa los resultados por unidad para mostrar el desempeño por tema.
interface UnitStat {
  name: string
  correctas: number
  total: number
...
function buildUnitStats(resultados: Resultado[]): UnitStat[] {
  ...
  for (const r of resultados) {
    const cur = map.get(r.unitName) ?? { correctas: 0, total: 0 }
    cur.total += 1
    if (r.correcta) cur.correctas += 1
    map.set(r.unitName, cur)
  }
  return Array.from(map.entries()).map(([name, s]) => ({
    name,
    correctas: s.correctas,
    total: s.total,
    pct: s.total > 0 ? Math.round((s.correctas / s.total) * 100) : 0,
  }))
}

// Color semántico según el desempeño: fuerte / medio / flojo.
...
function UnitBreakdown({ resultados }: { resultados: Resultado[] }) {
  ...
  return (
    <DarkCard className="p-6 sm:p-8">
      ...
      <p className="mt-1 text-sm text-white/48">
        Cómo te fue en cada tema según las preguntas que te tocaron.
      </p>
      <div className="mt-6 space-y-5">
        {stats.map((u) => {
          const tone = unitTone(u.pct)
          const flojo = u.pct < 60
          return (
            <div key={u.name}>
              <div className="flex items-baseline justify-between gap-3">
                ...
                <span className="shrink-0 text-sm tabular-nums text-white/52">
                  {u.correctas}/{u.total}{' '}
                  <span className={cn('font-bold', tone.text)}>{u.pct}%</span>
                </span>
              </div>
